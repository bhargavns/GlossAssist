...
function AdminConsolePage() {
  ...
  const handleStatusUpdate = async (requestId, status) => {
    ...
    try {
      const response = await updateCodeRequestStatus(requestId, status);

      if (status === 'approved' && response?.autoGeneratedCode?.code) {
        const expiryText = response.autoGeneratedCode.expiresAt
          ? new Date(response.autoGeneratedCode.expiresAt).toLocaleString()
          : 'Not set';

        window.alert(
          `Access request approved.\n\n` +
          `Generated code: ${response.autoGeneratedCode.code}\n` +
